...
class Ghost:
    ...
    def move_clyde(self):
        # r, l, u, d
        # clyde збирається повернути, коли буде вигідно для переслідування
        if self.direction == 0:
    ...
    def move_blinky(self):
        # r, l, u, d
        # blinky збирається повертати щоразу, коли зіткнеться зі стінами, інакше продовжує прямо
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
    ...
    def move_pinky(self):
        # r, l, u, d
        # inky збирається повертати ліворуч або праворуч, щоб переслідувати, але лише вгору чи вниз у разі зіткнення
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
...
while run:
    ...
    turns_allowed = player.check_position(center_x, center_y)
    if moving:
    ...
    score, powerup, power_counter, eaten_ghost = drawing.check_collisions(score, powerup, power_counter, eaten_ghost)
